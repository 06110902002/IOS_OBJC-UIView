...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id <UIViewControllerTransitionCoordinator>)coordinator {
    ...
    [coordinator animateAlongsideTransition:^(id <UIViewControllerTransitionCoordinatorContext> context) {
        if (self && [self presentable]) {
            ...
            NSLog(@"137-----------viewWillTransitionToSize");
...
- (void)layoutPresentedView:(UIView *)containerView {
    ...
    [containerView addGestureRecognizer:self.handler.panGestureRecognizer]; //传递手势
...
- (HWPanContainerView *)panContainerView {
    if (!_panContainerView) {
        ...
        _panContainerView.backgroundColor = [UIColor brownColor];
    }
