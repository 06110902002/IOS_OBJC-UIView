...
//重新布局子控件的位置，
//note : 当重新布局子视图位置时，需要[super layoutSubViews]
//
-(void) layoutSubviews{
...
/**
 note1: 如果本接口不被重写，那么-(void) drawLayer:(CALayer *)layer inContext:(CGContextRef)ctx
        接口也不会被调用
...
/**
 ...
 @param rect 画面尺寸
 */
-(void) drawTriangleInDrawRect:(CGRect) rect{
...
//override method
//UIView中的CALayer层的画图接口


/**
 
 note1:
    如果本类中重载本接口，那么渲染将优先本接口逻辑
 
 note2:
    如果显示调用了 [super drawLayer:layer inContext:ctx];
    那么- (void)drawRect:(CGRect)rect也将被调用
 
 note 3:
    如果本接口重载了，但是未显示调用 [super drawLayer:layer inContext:ctx];
    那么- (void)drawRect:(CGRect)rect将不被调用
 **/
